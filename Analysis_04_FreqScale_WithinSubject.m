...
outTxt = ['Correlation SSS/Consistency: BF10=',num2str(BF10_SSS_BAcc),', r=',num2str(r_SSS_BAcc),', p=',num2str(p_SSS_BAcc)];
disp(outTxt) ;
fprintf(fid, [outTxt '\n']);
...
for iFile = 1:length(vecSubject) 
    ...
    plot_sr(toPlot,20*[-1e-3 1e-3], 'Scale (cyc/oct)', 'Frequencies (Hz)',... 
                       XTick_pos, XTick_labels,...
                       YTick_pos, YTick_labels, 1) ;    
    title(num2str(iSubject));
    saveas(gcf,['./out/FreqScale/MaskBySubject/',num2str(iFile),'_sub#_',num2str(iSubject)],'epsc')
end
close all;
...
saveas(gcf,['./out/FreqScale/subjectMasks_sorted_from_PC1'],'epsc')
close all;
...
saveas(gcf,['./out/FreqScale/idealizedPCA_PC1'],'epsc')
close all;
...
saveas(gcf,['./out/FreqScale/idealizedPCA_PC2'],'epsc')
...
plot_sr(toPlot,.2*[-9e-3 9e-3], 'Scale (cyc/oct)', 'Frequency (Hz)',... 
                   XTick_pos, XTick_labels,...
                   YTick_pos, YTick_labels, 1) ;              
saveas(gcf,['./out/FreqScale/idealizedPCA_variance_PC1'],'epsc');
...
plot_sr(toPlot,.2*[-9e-3 9e-3], 'Scale (cyc/oct)', 'Frequency (Hz)',... 
                   ...
                   YTick_pos, YTick_labels, 1) ;
colormap(customcolormap_preset('red-white-blue'));
saveas(gcf,['./out/FreqScale/idealizedPCA_variance_PC2'],'epsc');
close all;
...
legend('PC1','PC2')
axis('square')
saveas(gcf,['./out/FreqScale/idealizedPCA_variance_marginals_scale'],'epsc')

figure;
toPlot_PC1 = interp2(rot90(PC1),4) ;
toPlot_PC2 = interp2(rot90(PC2),4) ;
XTick_pos = linspace(1,numrc(toPlot,1),8) ;
...
saveas(gcf,['./out/FreqScale/idealizedPCA_variance_marginals_frequencies'],'epsc')
%close all;
