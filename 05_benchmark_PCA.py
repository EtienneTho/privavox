...
for iComponent, n_components in enumerate(n_components_tab):  
  path = './out/benchmark_pca/'+str(n_components)+'_PCs/'
  ...
  for iSubject in [1, 2, 3]:
    ...
    for iRepeat in range(Ntimes):
      ...
      pca_optimal_dim = PCA(n_components=n_components, svd_solver='auto', whiten=True).fit(tabStrf[tabSubjectNb!=(iSubject)][:])
    ...
    sio.savemat(path+str(iSubject).zfill(3)+'_AllMaps_3d.mat', {'canonicalAllMaps': np.asarray(tabInterpBySubject), 'iSubject': iSubject, 'nDim_optimal_pca_tab':np.asarray(nDim_optimal_pca_tab), 'explained_variance':np.sum(pca_optimal_dim.explained_variance_ratio_)})
